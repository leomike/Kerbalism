	...
	public class Reliability : PartModule, ISpecifics, IModuleInfo, IPartCostModifier, IPartMassModifier
	{
		...
		public void FixedUpdate()
		{
			...
			if (!broken && mtbf > 0 && PreferencesReliability.Instance.mtbfFailures)
			{
				...
				{
					last = now;
					// using an exponential distribution resulting in a constant MTBF over the lifetime of the part
					next = now + mtbf * (quality ? Settings.QualityScale : 1.0) * Math.Log(Lib.RandomDouble());
#if DEBUG_RELIABILITY
		...
		protected void RunningCheck()
		{
			...
			if (running && rated_operation_duration > 1 && lastRunningCheck > 0)
			{
				...
				// adjust aging based on throttle, with higher throttle causing significantly more wear
				operation_duration += duration * Math.Pow(Math.Max(0.01, GetThrottle()), 2);
				vessel.KerbalismData().ResetReliabilityStatus();
				...
				if (fail_duration <= 0)
				{
					...
					// using an exponential distribution resulting in a constant MTBF over the lifetime of the part
					fail_duration = rated_operation_duration * (quality ? Settings.QualityScale : 1.0) * (1 - Math.Log(Lib.RandomDouble()) / PreferencesReliability.Instance.engineOperationFailureChance);
#if DEBUG_RELIABILITY
		...
		public static void BackgroundUpdate(Vessel v, ProtoPartSnapshot p, ProtoPartModuleSnapshot m, Reliability reliability, double elapsed_s)
		{
			...
			{
				// using an exponential distribution resulting in a constant MTBF over the lifetime of the part
				next = now + reliability.mtbf * (quality ? Settings.QualityScale : 1.0) * Math.Log(Lib.RandomDouble());
